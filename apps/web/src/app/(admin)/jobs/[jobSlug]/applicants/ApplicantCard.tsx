...
type ApplicantStatusWithoutPending = Exclude<ApplicantStatus, 'PENDING'>;
...
export default function ApplicantCard({ applicant }: { applicant: any }) {
	...
	const handleStatusChange = async (newStatus: ApplicantStatusWithoutPending) => {
		try {
			setIsUpdating(true);
	...
	const getStatusOptions = (currentStatus: Applicant['status']):Array<ApplicantStatusWithoutPending> => {
						...
						<div className="flex flex-col gap-2 w-full max-w-[250px]">
							<div className="dropdown dropdown-end w-full">
								...
								{isOpen && (
									...
									>
										{getStatusOptions(applicant.status).map((option:ApplicantStatusWithoutPending) => (
											<li key={option}>
												<button
