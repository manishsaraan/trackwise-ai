...
const ApplicationDetailsModal = ({ isOpen, onClose, applicant }: ApplicationDetailsModalProps) => {
		...
		<div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-end">
			<div className="w-[70%] bg-base-100 h-full overflow-y-auto">
				...
				<div className="p-6 space-y-8">
					...
					<div className="space-y-4">
						<h3 className="text-lg font-semibold border-b border-base-200 pb-4">Questionnaire Responses</h3>
						<div className="space-y-6">
							{applicant.questions.map((qa, index) => (
								<div key={index} className="space-y-3">
									<h4 className="font-medium text-base-content/70">{qa.question}</h4>
									<p className="text-base-content whitespace-pre-wrap">{qa.answer}</p>
									{index < applicant.questions.length - 1 && <div className="border-b border-base-200 pt-3" />}
								</div>
							))}
						</div>
					</div>
